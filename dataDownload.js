	...
	mindgatev3.get_token(options)
	  ...
	  .then(function(tsResponse) {
	  	...
	  	tsResult.forEach(function(snapshot, index){
	  		//let temp = Math.abs(tsResult[index].Total_Reactive_Power);
	  		tsArray.push(snapshot._time);
	  		activePowerArray.push(Number.parseFloat(snapshot.Total_Active_Power / 1000).toFixed(1));
	  		reactivePowerArray.push(Math.abs(Number.parseFloat(snapshot.Total_Reactive_Power / 1000).toFixed(1)));
	  		apparentPowerArray.push(Number.parseFloat(snapshot.Total_Apparent_Power / 1000).toFixed(1));
	  	})
